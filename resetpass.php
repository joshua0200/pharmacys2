...
<script>
	...
	$('#verify-form').submit(function(e) {
		...
		if ($(this).find('.alert-danger').length > 0)
			$(this).find('.alert-danger').remove();
		$.ajax({
			url: 'ajax.php?action=verifyReset',
			method: 'POST',
			data: {
				email: $("#email").val(),
				code: $("#code").val()
			...
			success: function(resp) {
				...
				if (resp == 1) {
					$("#code-error").remove();
					$('#verify-form').prepend('<div id="code-error" class="alert alert-success">Success. New password is sent to your email.</div>');
