...
if (isset($_GET['id'])) {
	$inv = $conn->query("SELECT * FROM product_list p where p.id=" . $_GET['id']);
}
?>
							...
							<div class="col-md-4">
								...
								<select name="" id="product" class="custom-select browser-default select2">
									...
									while ($row = $product->fetch_assoc()) :
										$prod[$row['id']] = $row;
									?>
										<option value="<?php echo $row['id'] ?>" data-mease="<?= $row['measurement'] ?>" data-mode="<?= $row['selling_mode'] ?>" data-name="<?php echo $row['name'] ?>" data-description="<?php echo $row['description'] ?>" data-price="<?= $row['price'] ?>"><?php echo $row['name'] . ' • ' . $row['measurement'] . ' • [' . $row['sku'] . ']' . ' • by ' . $row['selling_mode'] . '' ?></option><?php endwhile; ?>
								</select>
							</div>
							<div class="col-md-2">
									...
									?>
											<tr class="item-row" data-id="<?= $row['id'] ?>">
												...
												<td>
													<div class="row-5">
														<div class="col">
															<input type="number" min="1" step="any" name="qty[]" value="1" class="text-center form-control">
														</div>
														<div class="col">
