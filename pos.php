			...
			<div class="card-body">
				<form action="" id="manage-sales">
					...
					<div class="col-md-12">
						...
						<div class="row mb-3">
							<div class="col-md-12 pb-3">
								...
								<select name="" id="product" class="custom-select browser-default select2">
									<option value=""></option>
									<?php
									$product = $conn->query("SELECT *, i.id as item_id FROM inventory i INNER JOIN product_list p ON i.product_id = p.id  WHERE expiry_date > '" . date('y-m-d') . "' AND qty > 0 order by name asc");
									while ($row = $product->fetch_assoc()) :
									?>
										<option value="<?php echo $row['item_id'] ?>" data-name="<?php echo $row['name'] ?>" data-measurement="<?php echo $row['measurement'] ?>" data-description="<?php echo $row['description'] ?>"><?php echo '[Barcode: ' . $row['sku'] . '] • ' . $row['name'] . ' • ' . $row['measurement'] . ' • [Batch #' . $row['batch_no']  . '] • [' . $row['qty'] . ' Available]' ?></option>
									<?php endwhile; ?>
								</select>
							...
							<div class="col-md-3">
								...
								<div>
									<button class="btn btn-success" type="button" id="add_list"><i class="fa fa-plus"></i> Add to List</button>
								</div>
							</div>
						</div>
						<div class="mx-1">
						...
						<div class="row">
							<div class="col-lg-12">
								<button class="btn col-sm-3 btn-block float-right " style="background-color: #e35e05; color: white;" type="button" id="pay">Check out</button>
							</div>
						</div>
					</div>
...
<div id="tr_clone">
	<table>
		<tr class="item-row">
			...
			<td>
				<input type="number" min="1" step="any" name="qty[]" value="" class="text-center form-control" required>
			</td>
			<td>
...
<div class="modal fade" id="void_modal" role='dialog'>
	<div class="modal-dialog modal-md" role="document">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" id='submit' onclick="rem_list($(this))">Confirm</button>
				<button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
			</div>
		</div>
	</div>
...
<script>
	...
	$('#add_list').click(function() {
		...
		$.ajax({
			...
			success: function(resp) {
				...
				if (parseInt(resp.available) >= parseInt(qty)) {
					...
					tr.find('[name="qty[]"]').attr('max', parseInt(resp.available))
	...
	function calculate_total() {
		...
		$('#list tbody').find('.item-row').each(function() {
			...
			if (parseInt(_this.find('[name="qty[]"]').val()) > parseInt(_this.find('[name="qty[]"]').attr('max'))) {
				_this.find('[name="qty[]"]').val(_this.find('[name="qty[]"]').attr('max'))
				alert_toast("Product quantity is greater than available stock.", 'danger')
			}
